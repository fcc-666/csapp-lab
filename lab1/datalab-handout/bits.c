 ...
 *   Example: conditional(2,4,5) = 4 *   Legal ops: ! ~ & ^ | + << >>
 *   Max ops: 16
 *   Rating: 3
 */
...
int isLessOrEqual(int x, int y) {
  ...
  int case4=signx&(!signy);
  return case1||(case3&&(case2||case4));
}
//4
/* 
...
int howManyBits(int x) {
  ...
  x>>=bit1;
  bit0=x;
  int sum=bit0+bit1+bit2+bit4+bit8+bit16+1;
  return sum;
}
//float
/* 
...
unsigned float_twice(unsigned uf) {
  ...
  if(!(e^0xff)) return uf;
  e++;
  if(!(e^0xff)) return 0x7f800000|sign;
